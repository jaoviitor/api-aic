...
// Configurações do Blob Storage usando as variáveis gerais do Azure Storage
const CONNECT_STR = process.env.AZURE_STORAGE_CONNECTION_STRING || '';
const CONTAINER_NAME = process.env.AZURE_STORAGE_COUNTAINER_NAME_PDF || 'vital-pdfs';

// Verificar se as credenciais do Azure estão disponíveis
const hasAzureCredentials = CONNECT_STR && CONTAINER_NAME;

// Inicializar o cliente do Blob Storage apenas se as credenciais estiverem disponíveis
let BLOB_SERVICE_CLIENT;
if (hasAzureCredentials) {
    BLOB_SERVICE_CLIENT = BlobServiceClient.fromConnectionString(CONNECT_STR);
}
...
// Verificar se as credenciais do banco de dados estão disponíveis
const hasDbCredentials = HOST && PORT && USER && PASSWORD && DATABASE;
...
async function getAllEvaluationsFromDb() {
    // Verificar se as credenciais do banco de dados estão disponíveis
    if (!hasDbCredentials) {
        console.error('Credenciais do banco de dados não configuradas');
        return null;
    }
...
/**
* Salva uma imagem no Azure Blob Storage ou localmente se o Azure não estiver disponível
*/
async function saveImageToBlobCv(imageBuffer: Buffer, filename: string): Promise<string> {
    // Verificar se as credenciais do Azure estão disponíveis
    if (!hasAzureCredentials) {
        // Salvar localmente como alternativa
...
/**
* Processa a requisição de análise de imagem
*/
export const processVitalCv = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!urlImage) {
            res.status(400).send("URL da imagem não foi fornecida.");
            return;
        }
        
        if (!idPatient) {
            res.status(400).send("ID do paciente não foi fornecido.");
            return;
        }
        ...
        if (!hasAzureCredentials) {
            res.status(503).json({
                message: 'Azure Blob Storage service is not available',
        ...
        if (!hasHighProbability) {
            res.status(404).send("Nenhum membro detectado ou imagem não corresponde a um membro.");
            return;
        }
        ...
        if (totalWounds === 0) {
            res.status(200)
               ...
               .send("Nenhuma ferida detectada ou imagem não corresponde a ferida.");
            return;
        }
